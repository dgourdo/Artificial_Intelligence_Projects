...
def expand_front(front, method):  # Επέκταση μετώπου
    if method == 'DFS':
        ...
        if front:
            ...
            node = front.pop(0)
            for child in find_children(node):
                front.append(child)           # !!! Διαφορά σε σχέση με τον DFS:
                                              # Δημιουργία νέων παιδιών και τοποθέτησή τους στο τέλος του μετώπου.
    return front
...
def extend_queue(queue, method):  # Επέκταση ουράς
    if method == 'DFS':
        ...
        for child in children:
            path = copy.deepcopy(node)
            path.append(child)
            queue_copy.append(path)    # !!! Διαφορά σε σχέση με τον DFS:
                                       # Δημιουργία νέων παιδιών στο τέλος της ουράς αναζήτησης.
    return queue_copy
